        ...
        <>
            <Box display={"flex"} flexDirection={"column"} alignItems={"flex-start"}>
                ...
                <Box display="flex" mt={1}>
                    <Button component="label" color="secondary" variant="contained" sx={{ height: 36.5 }}>
                        파일 선택
                        <input type="file" accept="image/x-png, image/gif, image/jpeg" onChange={onChangeChooseImg} hidden />
                    </Button>
                    {newImgDir ? (
                        <Box ml={2} component="img" alt="newImg" src={typeof newImgDir === "string" ? newImgDir : undefined} sx={{ width: "auto", height: 100 }} />
                    ) : (
                ...
                ...old,
                {
                    ...
                    component: (
                        <Box display={"flex"} alignItems={"center"} gap={1}>
                            ...
                            <DeleteIcon sx={{ mt: 3 }} fontSize="small" onClick={() => onRemove(id)} />
                        </Box>
                    ),
                },
            ]);
            ...
            <Modal open={openAdd} onClose={handleCloseAdd}>
                <Box sx={style}>
                    <Box display="flex" justifyContent="space-between">
                        <Typography sx={{ fontWeight: "600", fontSize: "1.1rem", pb: 1 }}>활동 추가</Typography>
                    </Box>
                    ...
                    <Box display="flex" justifyContent="space-between" mt={2}>
                        ...
                        <Box display="flex" gap={1.5}>
                            <Button onClick={handleCloseAdd} color="secondary" variant="outlined" sx={{ fontWeight: "600" }}>
                                취소
                            </Button>
                            <Button color="secondary" variant="contained" sx={{ fontWeight: "600" }} onClick={handleSubmit(onValid)}>
                                추가
                            </Button>
                        </Box>
                    </Box>
            ...
            <Dialog open={open} onClose={handleClose} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
                <form onSubmit={itemHandleSubmit}>
                    <DialogContent>
                        <DialogContentText id="alert-dialog-description">
                            <Box display={"flex"} alignItems={"center"}>
                                <Typography sx={{ pr: 2, color: "#222222" }}>항목 이름</Typography>
                                <TextField
                                    color="secondary"
                                    InputProps={{ disableUnderline: true }}
                                    hiddenLabel
                            ...
                            <Box display={"flex"} alignItems={"center"} sx={{ mt: 2 }}>
                                ...
                                <RadioGroup row aria-labelledby="demo-row-radio-buttons-group-label" name="row-radio-buttons-group" defaultValue={0} onChange={onChangeType}>
                                    <FormControlLabel value={0} control={<Radio />} label="텍스트" />
                                    <FormControlLabel value={1} control={<Radio />} label="링크" />
                                    <FormControlLabel value={2} control={<Radio />} label="이미지" />
                                    <FormControlLabel value={3} control={<Radio />} label="날짜" />
                                </RadioGroup>
                            </Box>
                        </DialogContentText>
                    </DialogContent>
                    <DialogActions>
                        ...
                        <Button onClick={handleClose} color="secondary" sx={{ fontWeight: 600 }} variant={"contained"} autoFocus type="submit">
                            항목 추가
                        </Button>
                    </DialogActions>
                </form>
