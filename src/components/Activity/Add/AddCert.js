...
export default function AddCert() {
  ...
  const getDate = (event) => {
    setJson(JSON.stringify({ date: event.target.value }));
  };

  const getAgency = (event) => {
        ...
        <Box sx={style}>
          <Box>
            ...
            <Box maxHeight={450} overflow="auto" pb={1}>
              ...
              <InputLabel sx={{ mt: 1 }}>자격 및 면허명</InputLabel>
              <TextField
                color="secondary"
                InputProps={{ disableUnderline: true }}
                fullWidth
              ...
              <InputLabel sx={{ mt: 1 }}>인정기관명</InputLabel>
              <TextField
                color="secondary"
                InputProps={{ disableUnderline: true }}
                fullWidth
